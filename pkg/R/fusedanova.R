...
##' \code{\link{plot.fusedanova}} and \code{\link{cv.fa}}.
##' @name fusedanova
##' @rdname fusedanova
...
fusedanova <- function(x,class,...) {
	...
	## ====================================================
	## CONVERSIONS OF x in a list and y in a numeric vector
	...
	if (args$split==1){
		algoType = "No Split"
		if(args$verbose){cat("\nPath calculated with path without split")}
	}else{
		algoType = "With possible Splits"
		if(args$verbose){cat("\nPath calculated with path with possible splits")}
	}
