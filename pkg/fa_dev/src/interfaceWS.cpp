...
SEXP tree2listWS(Group *g, int graphSize){
...
SEXP tree2listWS(Group *g, int graphSize, NumericVector lambdalist){
...
RcppExport SEXP withSplit(SEXP R_x,SEXP R_xv,SEXP R_ngroup,SEXP R_args){
	...
	if (lambdalist.length()==0){
		...
		L = tree2listWS(G,graphSize);  // tree conversion 
	}else{
		if(verbose){
			Rprintf("Creating result and prediction Dataframe. \n");
		}
		L = tree2listWS(G,graphSize,lambdalist);
	}
